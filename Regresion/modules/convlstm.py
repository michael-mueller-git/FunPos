...
class ConvLSTMCell(nn.Module):
    ...
    Args:
        img_size (int, int): image size in (hight, width)
        input_dim (int): Number of channels of input tensor.
        hidden_dim (int): Number of channels of hidden state.
        kernel_size (int, int): Size of the convolutional kernel for both cnn and rnn.
        cnn_dropout (float): dropout rate for convolutional input.
        rnn_dropout (float): dropout rate for convolutional state.
        bias (bool): Whether or not to add the bias.
        peephole (bool): Add connection between cell state to gates
...
class ConvLSTM(nn.Module):
    ...
    Args:
        img_size (int, int): image size in (height, with)
        input_dim (int): Number of channels in input
        hidden_dim (int): Number of hidden channels
        kernel_size (int, int): Size of kernel in convolutions
        cnn_dropout (float): dropout rate for convolutional input.
        rnn_dropout (flaot): dropout rate for convolutional state.
        batch_first (bool): Whether or not dimension 0 is the batch or not
        bias (bool): Bias or no bias in Convolution
        peephole (bool): Add connection between cell state to gates
        layer_norm (bool): Use layer normalization
        return_sequence (bool): return output sequence or final output only
        bidirectional (bool): use bidirectional ConvLSTM
    """
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        Args:
            input_tensor (torch.tensor): 5-D Tensor either of shape (t, b, c, h, w) or (b, t, c, h, w)
            hidden_state: None, TODO implement stateful

        Returns:
            (layer_output, last_state, last_state_inv): if bidirectional else (layer_output)
        """
        ...
        return layer_output \
            ...
            else layer_output[:, -1:], last_state, last_state_inv \
            if self.bidirectional is True \
            else None
